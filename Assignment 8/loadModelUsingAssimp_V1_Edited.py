...
def printMeshInfo(mesh):
    ...
    print("number of Normal arrays: ", len(mesh.normals))
    if len(mesh.normals) > 0:
        print("first 3 normals:\n" + str(np.round(mesh.normals[:3], decimals=2)))
...
def getObjectDataList(filename, verbose=False):
    ...
    for mesh in scene.meshes:
        ...
        if len(mesh.normals) > 0:
            normals = np.asarray(mesh.normals, dtype="f4").reshape(-1, 3)
        else:
            print(f"Warning: No normals found for mesh {mesh.name}. Generating default normals.")
            normals = np.zeros_like(vertices)
            normals[:, 1] = 1.0  
        
        texcoords = np.asarray(mesh.texcoords[0], dtype="f4").reshape(-1, 2)
    
        geomData = np.concatenate((vertices, texcoords, normals), axis=1).astype("float32").flatten()
